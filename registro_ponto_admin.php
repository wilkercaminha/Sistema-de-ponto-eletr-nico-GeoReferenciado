    ...
    <script>
        ...
        function toggleMap(lat, lng, id) {
            ...
            if (!isMapVisible) {
                ...
                // Create a new map
                map = L.map('map').setView([lat, lng], 15); // Ajustar o zoom para 15 para maior precisão
                ...
                // Atualizar botões
                updateMapButtons(id, false);
            }
        }

        function updateMapButtons(id, isOpen) {
            const button = document.getElementById(`map-btn-${id}`);
            if (isOpen) {
